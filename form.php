...
@header('Status Code: 200 OK;');
	...
	function makeFormField($field, $cc_request = array(), $currentform = false) {
		...
		if(!empty($required)) {
			$required = " required";
			$reqlabel = isset($_REQUEST['text']['reqlabel']) ? $_REQUEST['text']['reqlabel'] : 'The %s field is required';
			$asterisk = isset($_REQUEST['reqast']) ? '<span class="req" title="'.sprintf($reqlabel, $label).'">*</span>' : '';
			if(!($t == 'ta' || $t == 'b' || $t == 's')) {
		...
		if($t == 'ta') {
			if(isset($_REQUEST['output']) && $_REQUEST['output'] == 'html') { $fields .= html_entity_decode(stripslashes(stripslashes($val))); } else { $fields .=  stripslashes(stripslashes($val)); }
		} else if($t == 'b' || $t == 's') {
	...
	function processForm() {
		...
		if(isset($_REQUEST['f'])) {
			if(isset($_REQUEST['text'])) {
				$_REQUEST['text'] = json_decode(stripslashes($_REQUEST['text']), true);
			}
			extract($_REQUEST);
		}
		...
		if(is_array($f)) {
			...
			$fields = '<div class="kws_input_fields">'.$fields.$listoutput.'</div>';
		}
