...
function constant_contact_create_object($force_new = false)
{
	...
	if((is_admin() && current_user_can('manage_options') && (isset($_GET['page']) && $_GET['page'] == 'constant-contact-api' && isset($_GET['settings-updated']) || !get_option('cc_configured'))) || empty($username) || empty($password)) {
		$force_new = true;
	}
	...
	if(is_object($new_cc) && $new_cc->get_service_description()) {
		...
		set_transient('cc_object', maybe_serialize($new_cc), 60*60*12);
...
function constant_contact_public_signup_form($args, $echo = true) {
	...
	if(constant_contact_get_transient($unique_id) === 'success') {
		...
		// Reset this form
		constant_contact_set_transient($unique_id, false);
...
function constant_contact_get_akismet_fields() {
    ...
    $akismet_info['user_agent']   = substr($_SERVER['HTTP_USER_AGENT'], 0, 254);
...
function constant_contact_current_page_url() {
	 ...
	 return remove_query_arg('success',$pageURL);
}
...
function constant_contact_get_transient($key = false) {
	   if(!isset($_REQUEST['cache'])) {
	   	...
	   	if(function_exists('set_site_transient')) {
			$transient = get_site_transient('cc_api');
		} else {
...
function constant_contact_set_transient($key = false, $data, $time = 172800) {
	...
	if(!is_admin()) {
		if(!empty($key)) {
			$transient = (array)constant_contact_get_transient();
			$transient[$key] = $data;
			// Set a cached version of the table so it'll be faster next time. Expires every 12 hours
			if(function_exists('set_site_transient')) {
				set_site_transient('cc_api',$transient, $time);
			} else {
