...
function ccfg_compatibility_warning() {
	if(check_ccfg_compatibility() === false && floor($GLOBALS['wp_version'] > 3)) { ?>
	...
	<?php
		...
		<div class="error"><p><strong><?php _e('The Constant Contact API plugin must be configured properly before using the Form Generator.', 'constant-contact-form-generator' ); ?></strong></p></div>
	<?
		return false;
...
function constant_contact_retrieve_form($formid, $force_update=false, $unique_id = '') {
	...
	if(!$success && empty($_GET) && empty($_POST) && !$force_update && $form = get_transient("cc_form_$formid")) {
		return $form;
	}
	...
	$response = wp_remote_post(CC_FORM_GEN_PATH.'form.php', array('body' => $list));
...
function cc_form_get_selected_id($allForms = array()) {
	...
	if(empty($allForms)) {
		$cc_form_selected_id = isset($_REQUEST['cc-form-id']) ? $_REQUEST['cc-form-id'] : -1;
	} else {
...
//	wp_enqueue_script( 'form-menu', plugin_dir_url(__FILE__).'js/form-menu.dev.js' );
	...
	$params = array('path' => plugin_dir_url(__FILE__), 'rand' => mt_rand(0, 10000000), 'text' => cc_form_text());
						...
						<div id="submitpost" class="submitbox">
							<div class="major-publishing-actions">
								...
								<div class="delete-action">
									<a class="submitdelete deletion menu-delete" href="<?php echo esc_url( wp_nonce_url( admin_url('admin.php?page=constant-contact-forms&action=delete&amp;form=' . $cc_form_selected_id), 'delete-cc_form-' . $cc_form_selected_id ) ); ?>"  onclick="return confirm('<?php _e('Are you sure you want to delete this form?'); ?>');"><?php _e('Delete Form'); ?></a>
