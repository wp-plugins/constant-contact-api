...
function constant_contact_campaigns()
{
	...
	if(isset($_GET['add'])) {
		...
		if(isset($_POST['add_campaign']) && wp_verify_nonce($_POST['add_campaign'],'cc_campaign')) {
			...
			if(!empty($_POST['post_id'])) {
				...
				$errors['id'] = 'Please select a post.';
			}
		}
	...
	?>
		<div class="wrap nosubsub">
			<h2 class="cc_logo"><a class="cc_logo" href="<?php echo admin_url('admin.php?page=constant-contact-api'); ?>">Constant Contact Plugin &gt;</a> <a href="<?php echo remove_query_arg(array('add')); ?>">Campaigns</a> &gt; Add New Campaign</h2>
			
			<p>Select a post, then submit the form. When the form is submitted, the content of the post will be sent to Constant Contact as a new Email. You can continue editing the email in Constant Contact.</p>
...
?>
				<p class="submit">
					...
					<input type="submit" value="Add Campaign" class="button-primary" /> <a href="<?php echo remove_query_arg('add'); ?>" class="button-secondary alignright">Cancel &amp; Return to Contacts Page</a>
				</p>
			</form>
<?php
	...
	<?php 
		...
		<div class="wrap nosubsub">
			<h2 class="cc_logo"><a class="cc_logo" href="<?php echo admin_url('admin.php?page=constant-contact-api'); ?>">Constant Contact Plugin &gt;</a> Campaigns <a href="<?php echo add_query_arg(array('add' => 1), remove_query_arg('refresh')); ?>" class="button add-new-h2" title="Add New Contact">Add New</a></h2>
			<?php constant_contact_admin_refresh(); ?>
