...
function constant_contact_contacts()
{
	...
	$contacts = $notices = $errors = $hidereviewerrors = array();
	...
	if(isset($_GET['add'])) {
		...
		if(isset($_POST['add_contact']) && wp_verify_nonce($_POST['add_contact'],'cc_contact')) {
			...
			if(isset($_POST['EmailAddress']) && is_email($_POST['EmailAddress']) && isset($_POST['lists']) && !empty($_POST['lists'])) {
				...
				if(is_string($contact) && preg_match('/error\ [0-9]+?/ism', $contact)) {
					$errors[] = $contact;
					$hidereviewerrors = true;
				}
		...
		if(!empty($_POST['EmailAddress'])) {
			if(empty($errors)) {
				if(!empty($id)) {
					$link = '<a href="'.admin_url('admin.php?page=constant-contact-contacts&id='.$id).'">View Contact Details</a>.';
				} else {
					$link = '<a href="'.admin_url('admin.php?page=constant-contact-contacts').'">Return to Contacts</a>.';
				}
				echo sprintf('<div class="updated"><p>Success: Contact has been added. %s</p></div>', $link);
				...
				if(empty($hidereviewerrors)) {
					echo '<p>'.__('Please review the form for <span style="color:red;">Errors in red</span>.','constant-contact-api').'</p>';
				}
				echo '</div>';
			}
		}
							...
							if($alt == 'alt') { $alt = '';} else { $alt = 'alt'; }
								...
								if(in_array($id, $noteditable)) {
									...
									if(is_array($options)) {
										...
										$content = '<input type="text" id="'.sanitize_title($id.'-field').'" name="'.$id.'" maxlength="'.$inputlength.'" value="'.esc_html($value).'" class="widefat"'.$style.' />'.$help;
...
#	r($Contacts);
			...
			if(empty($_POST['id']) || !is_numeric($_POST['id']) || empty($_POST['EmailAddress']) || (isset($_POST['EmailAddress']) && !is_email($_POST['EmailAddress']))) {
				...
				if($updated) {
					...
					<div class="updated">
						...
						<p class="submit"><a href="'.add_query_arg(array('refresh' => 'contacts'), remove_query_arg(array('edit', 'id'))).'" class="button-primary">'.__('Return to Contact List','constant-contact-api').'</a> <a href="'.remove_query_arg('edit').'" class="button-secondary action">'.__('Return to Contact','constant-contact-api').'</a></p>
					</div>';
				} else {
					$errors[] = __('The update failed. Check the form below for errors. If there are no errors below, this may be because of a Constant Contact server issue.','constant-contact-api');
				}
			}
		...
		<div class="wrap nosubsub">
			...
			if(!$contact) {
				echo '<tr><td><p>';
				echo __('Contact Not Found', 'constant-contact-api');
				echo ' </p></td></tr>';
			} else {
					$html = $alt = '';
							...
							if($id == 'lists') {
								...
								foreach($lists as $key => $details) {
									$listhtml .= '
										<li>
											<label for="cc_lists'.$details['id'].'">
													<input name="lists[]" type="checkbox" value="'.$details['id'].'" '. checked(in_array($details['id'], $v), true, false) .' id="cc_lists'.$details['id'].'" /> ' . $details['Name'] . '
											</label>
										</li>';
								}
								...
								$v = htmlentities( htmlspecialchars_decode( $v ) );
								...
								switch($id) {
									...
									case "StateCode":
										$inputlength = 2;
										$help = sprintf(__('Must be a valid US/Canada State Code and must be consistent with CountryCode. See %s', 'constant-contact-api'), '<a href="https://ui.constantcontact.com/CCSubscriberAddFileFormat.jsp#states" target="_blank">https://ui.constantcontact.com/CCSubscriberAddFileFormat.jsp#states</a>');
										break;
									case "CountryCode":
										$inputlength = 2;
										$help = sprintf(__('Must be a valid two character, lower case, Country Code. See: %s', 'constant-contact-api'), '<a style="text-decoration: none;" rel="nofollow" href="http://constantcontact.custhelp.com/cgi-bin/constantcontact.cfg/php/enduser/std_adp.php?p_faqid=3614" target="_blank">http://constantcontact.custhelp.com/cgi-bin/consta<wbr>ntcontact.cfg/php/enduser/std_adp.php?p_faqid=3614</a>');
										break;
									case "PostalCode":
									case "SubPostalCode":
										$inputlength = 25;
								...
								if(!empty($errors) && isset($_POST["{$id}"]) && strlen($_POST["{$id}"]) > $inputlength) {
									$help = '<p class="howto error" style="color:red;">'.sprintf(__('The maximum length of this field is %d characters. You have entered %d characters.', 'constant-contact-api'), $inputlength, strlen($_POST["{$id}"])).'</p>';
								}
		...
		<div class="wrap nosubsub">
			<?php
			if(!empty($contact['Name'])) { $contactTitle = sprintf(__(": &ldquo;%s&rdquo;", 'constant-contact-api'), $contact['Name']);  } else { $contactTitle = sprintf(__(" #%s", 'constant-contact-api'), $id); }
			?>
			<h2 class="cc_logo"><a class="cc_logo" href="<?php echo admin_url('admin.php?page=constant-contact-api'); ?>"><?php _e('Constant Contact Plugin', 'constant-contact-api'); ?> &gt;</a> <a href="<?php echo admin_url('admin.php?page=constant-contact-contacts'); ?>"><?php _e('Contacts', 'constant-contact-api'); ?></a> &gt; <?php _e('Contact', 'constant-contact-api'); ?><?php echo $contactTitle; ?></h2>
			...
			<p class="submit"><a href="<?php echo admin_url('admin.php?page=constant-contact-contacts'); ?>" class="button-primary"><?php _e('Return to Contacts', 'constant-contact-api'); ?></a> <a href="<?php echo add_query_arg('edit', true); ?>" class="button-primary alignright" style="margin:0 0 0 8px;"><?php _e('Edit this Contact', 'constant-contact-api'); ?></a></p>
	<?php
/*
...
*/
	...
	<?php
		...
		?>
		<div class="wrap nosubsub">
		<h2 class="cc_logo"><a class="cc_logo" href="<?php echo admin_url('admin.php?page=constant-contact-api'); ?>"><?php _e('Constant Contact Plugin', 'constant-contact-api'); ?> &gt;</a> <?php _e('Contacts', 'constant-contact-api'); ?> <a href="<?php echo add_query_arg(array('add' => 1), remove_query_arg('refresh')); ?>" class="button add-new-h2" title="Add New Contact"><?php _e('Add New Contact', 'constant-contact-api'); ?></a></h2>
		<?php
